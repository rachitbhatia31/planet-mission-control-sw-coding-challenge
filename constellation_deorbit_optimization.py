...
satellite orbital dynamics, atmospheric drag, and thrust-assisted deorbiting.
"""
...
def convert_and_range_angle(degrees, min_angle_rad, max_angle_rad):
    ...
    # Wrap the angle within specified range
    angle_rad = np.arctan2(np.sin(angle_rad), np.cos(angle_rad))
    return angle_rad

# Function to atmospheric density at a given altitude
...
def drag_acceleration(r, v, rho, Cd, S, m0, mp):
    '''Compute drag acceleration based on satellite parameters and atmospheric conditions.

        Args:
            r (numpy.ndarray): Position magnitude in meters
            v (numpy.ndarray): Velocity magnitude in meters per seconds
...
def compute_acceleration(r, v, m0, m_propellant, S, Cd, F, burn_flag):
    ...
    # Drag force
    acc_drag = drag_acceleration(r, v, rho, Cd, S, m0, m_propellant)  # Drag force
    ...
    acc_gravity = -9.81  # m per seconds squared
    acc_rtn = np.array([acc_gravity, (acc_drag + acc_thrust), 0])  # RTN acceleration
...
def gauss_planetary_equations(t, y, S, m0, F, Isp, Cd, Initial_epoch, burn_start_time_epoch, burn_start_time_delta,
    ...
    # Compute the RTN acceleration
    acc_rtn = compute_acceleration(r, v, m0, m_propellant, S, Cd, F, burn_flag)

    # Calculate derivatives using the Gauss planetary equations
...
def run_simulation(satellite_data):
    ...
    # Calculate initial altitude and prepare initial state vector
